...
Если название таба не равняется ни одному из значений выше, то название таба и его хэш определяются по дефолту: ```default``` и ```goods``` соотвественно.
Если ситуация повторяется, но товары нужно записать в новый раздел, то к названию таба и хэшу добавляется порядковый номер: ```default_1``` и ```goods_1```, ```default_2``` и ```goods_2```, ...
